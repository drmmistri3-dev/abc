    ...
    setStudents(prev => prev.map(s => {
      if (s.id === studentId) {
        const updatedExamResults = s.examResults.map(er => 
          er.termName === termName ? { ...er, scores: updatedScores } : er
        );
...
const QuickConnect: React.FC<{ phone: string, name: string, schoolName: string }> = ({ phone, name, schoolName }) => {
  ...
  return (
    <div className="flex gap-4 mb-8">
      <a 
        href={`tel:${phone}`} 
        className="flex-1 flex items-center justify-center gap-3 py-4 bg-blue-50 text-blue-700 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-100 transition-all border border-blue-100 shadow-sm"
      >
      ...
      <a 
        ...
        className="flex-1 flex items-center justify-center gap-3 py-4 bg-emerald-50 text-emerald-700 rounded-2xl font-black uppercase tracking-widest hover:bg-emerald-100 transition-all border border-emerald-100 shadow-sm"
      >
           ...
           <div className="flex p-1.5 bg-slate-100 rounded-2xl gap-1 shadow-inner">
             {student.examResults.map((term, idx) => (
               <button 
                ...
                className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTermIndex === idx ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:bg-white/50'}`}
               >
                 {term.termName}
               </button>
             ))}
